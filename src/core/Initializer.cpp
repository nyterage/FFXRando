...
std::vector<chunk_t> initializer_t::chunkData( std::vector<char>& bytes, int size, bool is_gear_shop_data ) const
{
  std::vector<chunk_t> chunks;
  int index = 0;
  for (int i = 0; i < bytes.size(); i += size)
  {
    chunk_t chunk = chunk_t( bytes, i, i + size, index, is_gear_shop_data );
...
void initializer_t::initializeShopData( bool gear )
{
  ...
  genericExcelReader<shop_data_t>( path, gear ? gear_shop_data : item_shop_data, 34, gear );
}
...
void initializer_t::initializeAeonScalingData()
{
  ...
  genericExcelReader<aeon_scaling_data_t>( path, aeon_scaling_data, 44, false, true );
}
...
void initializer_t::initializeSphereGridData()
{
  for (int i = 0; i < 3; i++)
  {
    ...
    sphere_grid_data_t* data = new sphere_grid_data_t( bytes, static_cast< sphere_grid_type_t >( i ) );
